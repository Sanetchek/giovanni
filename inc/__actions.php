...
// Remove product-category base from generated URLs
...
add_filter('request', function($vars) {
    // Check if we're on a potential category page
    if (empty($vars['product_cat']) && !empty($_SERVER['REQUEST_URI'])) {
        $path = trim(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH), '/');
        $segments = explode('/', $path);

        // Skip if we're on a known WordPress page
        if (!empty($segments[0]) && !in_array($segments[0], ['wp-admin', 'wp-content', 'wp-includes'])) {
            // Check if this slug exists as a product category
            $term = get_term_by('slug', end($segments), 'product_cat');
            if ($term) {
                $vars['product_cat'] = $term->slug;
            }
        }
    }
    return $vars;
...
// Flush rewrite rules when the code is first used
// IMPORTANT: Comment this out after first use
// flush_rewrite_rules();