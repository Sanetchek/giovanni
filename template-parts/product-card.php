...
$image_url = $image ? wp_get_attachment_image_url($image, '372-372') : wc_placeholder_img_src('full');

// Get the second image from the product gallery, if available
$attachment_ids = $product->get_gallery_image_ids();
$second_image_id = !empty($attachment_ids) ? $attachment_ids[0] : '';
$second_image_url = $second_image_id ? wp_get_attachment_image_url($second_image_id, '372-372') : wc_placeholder_img_src('full');
?>
  ...
  <div class="product-link">
    <div class="product-image">
      <a href="<?php echo esc_url($permalink); ?>">
        <img src="<?php echo esc_url($image_url); ?>" alt="<?php echo esc_attr($title); ?>" width="372" height="372">
      </a>

      <div class="product-hidden product-hidden-img">
        <?php echo simpleLikes($product_id); ?>
        <img src="<?php echo esc_url($second_image_url); ?>" alt="<?php echo esc_attr($title); ?>" width="372" height="372">
      </div>
    </div>
